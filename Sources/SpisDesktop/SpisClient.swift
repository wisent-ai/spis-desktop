...
struct SpisClient: Sendable {
    ...
    func crawlStart(config: CrawlerStartConfig) async throws -> CrawlOperation {
        let output = try await runCrawlProcess(args: config.buildArguments())
    ...
    func crawlStatus(runId: String, record: String? = nil) async throws -> CrawlOperation {
        var args = ["crawl", "status", "--run", runId]
        if let record = record {
            args.append("--record")
            args.append(record)
        }
        let output = try await runCrawlProcess(args: args)
        guard output.succeeded else {
            throw NSError(domain: "CrawlOperation", code: Int(output.exitCode), userInfo: [
    ...
    func crawlResume(runId: String) async throws -> CrawlOperation {
        let output = try await runCrawlProcess(args: ["crawl", "resume", "--run", runId])
        guard output.succeeded else {
            throw NSError(domain: "CrawlOperation", code: Int(output.exitCode), userInfo: [
    ...
    func crawlImport(runId: String) async throws -> CrawlOperation {
        let output = try await runCrawlProcess(args: ["crawl", "import", "--run", runId])
        guard output.succeeded else {
            throw NSError(domain: "CrawlOperation", code: Int(output.exitCode), userInfo: [
    ...
    // MARK: - Crawl process execution

    private func runCrawlProcess(args: [String]) async throws -> ProcessResult {
        let process = Foundation.Process()
        guard let spisBinary = locateSpisBinary() else {
    ...
    private func locateSpisBinary() -> String? {
